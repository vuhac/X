  ...
  <div class="main">
    <div class="header-nav">
      <div class="header-nav-body clear-fix">
        <span>您好,欢迎来到凤凰国际</span>
        <span class="time">{{time}}</span>
        <nav class="nav">
          <ul class="clear-fix">
            <!-- <li><a target="_blank" :href="links.jiechiLink">防劫持教程</a></li> -->
            <li><a target="_blank" href="javascript: void(0)">网址导航</a></li>
            <li><a target="_blank" href="javascript: void(0)" @click="addFavorite('凤凰国际')">添加收藏</a></li>
            <li><a @click="openKefu">在线客服</a></li>
            <li><a target="_blank" href="/static/fhcp/html/download/index.html">手机APP下载</a></li>
            <li><a href="javascript:void(0)" @click="loginTest">免费试玩</a></li>
          </ul>
        </nav>
      </div>
    </div>
    <div class="header-main">
      <a class="logo" href="javascript: void(0)" @click="$router.push('/home');"></a>
      <div class="header-image" style="height: 120px; background-size: 450px 120px; background-position: 20px center;"></div>
      ...
      <div class="login-before clear-fix" v-if="!userinfo">
        ...
        <div class="m-top5">
          <input v-if="code_show" class="header-input w-100" size="mini" autocomplete="off" placeholder="验证码" maxlength="4" type="text" v-model="passKey.code"/>
          <img v-if="code_show" class="auth-image" :src="codeImg" alt="" @click="getCode">

          <img src="/static/fhcp/img/testplay.png" class="try" @click="$router.push('/home/agent')" style="cursor:pointer"/>
        </div>
      ...
      <div class="login-after" v-if="userinfo">
        <div class="row">
          <p><span>帐号</span>：<span>{{userinfo.userName}}</span></p>
        </div>
        <div class="row">
  ...
  .main {
    ...
    .header-main {
      ...
      .logo{
        ...
        width: 206px;
        height: 99px;
        background:url('/static/fhcp/img/logo.png') no-repeat left center;
        background-size: 100%;
      }
