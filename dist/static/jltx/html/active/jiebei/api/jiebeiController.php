...
class jiebeiController extends jiebeiModel
{
    ...
    public function iniClassList($clientType = 'pc')
    {
        ...
        if (empty($getBankTransferList)) $arr[] = '网银转帐';
        if (empty($qrWeixin)) $arr[] = '微信转帐';
        if (empty($qrZhifubao)) $arr[] = '支付宝转帐';
    ...
    public function balanceReturnAmount($huankuanmoney = 0)
    {
        ...
        if ($addHuankuan) {
            ...
            if ($updateUserMoney['status']=='success') {
                ...
                $this->toJson(['msg' => "提交成功!您还款前帐号余额为{$user_before_money}元,还款后的余额变更为{$biangengyue},请5分钟后在[借还记录]查看还款是否审核成功!"], 200, "");
            }
        }
    }
